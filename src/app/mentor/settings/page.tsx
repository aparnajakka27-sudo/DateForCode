  ...
  return (
    <main className="relative min-h-screen bg-bg-dark-950 developer-grid overflow-hidden noise-bg">
      ...
      <nav className="relative z-20 py-3 px-8 flex items-center border-b border-border-dark bg-bg-dark-900/60 backdrop-blur-xl gap-4">
        <button onClick={()=>router.push('/mentor/dashboard')} className="flex items-center gap-2 text-[var(--text-muted)] text-xs font-mono font-bold uppercase tracking-wider hover:text-[var(--text-primary)] transition-colors group">
        ...
        <div className="flex-1 max-w-lg mx-auto">
          <div className="relative">
            ...
            <input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="Search mentor credentials..." className="w-full pl-10 pr-12 py-2.5 rounded-lg bg-bg-dark-950 border border-border-dark text-sm text-[var(--text-primary)] focus:outline-none focus:border-accent-purple/50 focus:bg-bg-dark-950 transition-all placeholder:text-[var(--text-muted)] font-mono" />
            <button onClick={startVoice} className={`absolute right-3 top-1/2 -translate-y-1/2 w-7 h-7 rounded-lg flex items-center justify-center transition-all ${isListening?'bg-accent-purple text-white scale-110':'text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-border-dark'}`}><Mic className="w-3.5 h-3.5" /></button>
                ...
                <motion.div initial={{opacity:0,y:-5}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-5}} className="absolute top-full left-0 right-0 mt-2 bg-bg-dark-900 rounded-lg border border-border-dark shadow-2xl overflow-hidden z-50 max-h-72 overflow-y-auto font-mono">
                  {filteredResults.map((item,i) => (
                    <button key={i} onClick={()=>{setActiveTab(item.tab);setSearchQuery('');}} className="w-full flex items-start gap-3 px-4 py-3 text-left hover:bg-accent-purple/10 transition-colors border-b border-border-dark last:border-0">
                      ...
                      <div className="min-w-0"><p className="text-sm font-bold text-[var(--text-secondary)] truncate">{item.label}</p><p className="text-[10px] text-[var(--text-muted)] truncate">{item.desc}</p></div>
                    </button>
                  ))}
                </motion.div>
              )}
      ...
      <div className="relative z-10 flex min-h-[calc(100vh-57px)]">
        ...
        <div className="w-14 flex-shrink-0 bg-bg-dark-900 border-r border-border-dark flex flex-col items-center py-4 gap-1.5 relative z-30">
          {SIDEBAR_ITEMS.map((n)=>(
            <div key={n.label} className="relative group">
              <button onClick={()=>router.push(n.href)} className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${n.label==='Settings' ? 'bg-accent-purple/10 text-accent-purple border border-accent-purple/20' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-border-dark'}`}>
                <n.icon className="w-4 h-4" />
              </button>
              <div className="fixed ml-[3.5rem] -mt-8 px-3 py-1.5 rounded bg-bg-dark-900 border border-border-dark text-[var(--text-primary)] text-[11px] font-mono whitespace-nowrap opacity-0 invisible group-hover:opacity-100 group-hover:visible pointer-events-none transition-all duration-200 shadow-xl" style={{zIndex:9999}}>
                {n.label}
        ...
        <motion.div {...fadeUp(0.05)} className="w-52 flex-shrink-0 bg-bg-dark-900/40 backdrop-blur-md border-r border-border-dark p-4 relative z-10 font-mono">
          <h1 className="text-sm font-bold text-[var(--text-secondary)] mb-5 px-2 tracking-widest uppercase">&gt;_ Credentials</h1>
          <div className="space-y-1">
            {TABS.map((tab,i)=>(
              <motion.button key={tab.id} {...fadeUp(0.08+i*0.03)} onClick={()=>setActiveTab(tab.id)}
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-xs font-bold text-left transition-all leading-tight ${activeTab===tab.id?'bg-accent-purple/10 text-accent-purple border border-accent-purple/20':'text-[var(--text-muted)] hover:text-[var(--text-secondary)] hover:bg-border-dark/30 border border-transparent'}`}>
          ...
          <AnimatePresence mode="wait">
            {activeTab==='edit-profile' && (
              <motion.div key="ep" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} transition={{duration:0.3}} className="max-w-4xl">
                <h2 className="text-xl font-bold font-mono tracking-tight text-[var(--text-primary)] mb-2"><span className="text-accent-gold mr-2">👑</span>Edit Prestigious Profile</h2>
                <p className="text-sm text-[var(--text-secondary)] mb-8 font-mono">Configure custom titles, verified skill frameworks, and active guidance scopes.</p>
                ...
                <div className="bg-bg-dark-900 border border-border-dark rounded-xl p-8 relative overflow-hidden">
                  ...
                  <div className="mb-8">
                    <label className="text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)] mb-3 block font-mono">MENTOR_IDENTIFIER_SYMBOL</label>
                    <div className="flex items-center gap-6">
                      ...
                      <div className="flex-1 max-w-md space-y-4 font-mono">
                        <div>
                          <label className="text-[10px] text-[var(--text-secondary)] mb-1 block">FULL_NAME</label>
                          <input type="text" value={name} onChange={e=>setName(e.target.value)} className="w-full px-4 py-3 rounded-lg border border-border-dark text-sm text-[var(--text-primary)] focus:outline-none focus:border-accent-purple transition-all bg-bg-dark-950"/>
                        </div>
                        <div>
                          <label className="text-[10px] text-[var(--text-secondary)] mb-1 block">VERIFIED_PROFESSIONAL_TITLE</label>
                          <input type="text" value={title} onChange={e=>setTitle(e.target.value)} placeholder="e.g. Staff Engineer @ Netflix" className="w-full px-4 py-3 rounded-lg border border-border-dark text-sm text-[var(--text-primary)] focus:outline-none focus:border-accent-purple transition-all bg-bg-dark-950"/>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Skills Known */}
                  <div className="mb-6">
                    <div className="flex items-center justify-between mb-3"><label className="text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)] font-mono">KNOWN_SKILLSETS</label></div>
                  ...
                  {/* Skills Guide */}
                  <div className="mb-8">
                    <div className="flex items-center justify-between mb-3"><label className="text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)] font-mono">MENTORING_SCOPE_GUIDES</label></div>
                    <div className="flex flex-wrap gap-2 max-w-2xl font-mono">
                  ...
                  <div className="flex items-center gap-3 pt-6 border-t border-border-dark font-mono">
                    <button onClick={handleReset} className="btn-secondary-dev flex items-center gap-1.5 hover:border-accent-purple hover:text-[var(--text-primary)]"><RotateCcw className="w-3.5 h-3.5"/>Reset</button>
            ...
            {activeTab !== 'edit-profile' && (
               <motion.div key="other" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}} className="flex flex-col items-center justify-center h-64 text-[var(--text-muted)] font-mono text-xs border border-dashed border-border-dark rounded-xl bg-bg-dark-900/20">
                 <p>&gt;_ INTERFACE SECTION UNDER DEVELOPMENT</p>
                 <p className="text-[10px] text-[var(--text-muted)] mt-2">Certified database registries locked.</p>
               </motion.div>
            )}
          </AnimatePresence>
        </div>
...
export default function MentorSettings() {
  return (
    <React.Suspense fallback={
      <div className="min-h-screen bg-bg-dark-950 flex flex-col items-center justify-center font-mono text-xs text-[var(--text-muted)] tracking-widest">
